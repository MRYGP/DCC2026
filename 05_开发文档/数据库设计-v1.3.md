...
CREATE TABLE tenants (
    ...
    -- V1轻复制新增字段（V0可不实现）
    partner_id VARCHAR(20) UNIQUE,  -- 专属ID（M001, M002...）
...
| 字段 | 类型 | 说明 | V0 | V1 | 示例 |
|------|------|------|----|----|------|
| partner_id | VARCHAR(20) | 专属ID（唯一） | ❌ | ✅ | M001 |
| brand_name | VARCHAR(100) | 品牌名称 | ❌ | ✅ | XX贷款顾问 |
| logo_url | VARCHAR(500) | Logo地址 | ❌ | ✅ | https://xxx/logo.png |
| contact_phone | VARCHAR(20) | 联系电话 | ❌ | ✅ | 138****1234 |
| contact_wechat | VARCHAR(50) | 微信号 | ❌ | ✅ | xxx |
| contact_address | VARCHAR(200) | 联系地址 | ❌ | ✅ | 长沙市 |
| theme_color | VARCHAR(20) | 主题色 | ❌ | ✅ | #1890ff |

**轻复制机制说明**：
```
客户访问：domain.com?partner_id=M001
  ↓
系统查询tenants表 → 找到M001的品牌配置
  ↓
页面显示：
  ├─ Logo：logo_url
  ├─ 名称：brand_name
  ├─ 联系方式：contact_phone, contact_wechat
  └─ 主题色：theme_color
  ↓
客户填写数据 → 自动标记source_partner_id = M001
  ↓
数据归属：middleman_id = M001对应的tenant_id
```
...
CREATE TABLE customers (
    ...
    reg_capital REAL,                    -- 注册资本（元，与13字段SSOT一致）
    established_date DATE,               -- 成立时间
    legal_person TEXT,                   -- 法人姓名
    
    annual_invoice REAL,                 -- 年开票（元）
    annual_tax REAL,                     -- 年纳税（元）
    monthly_flow REAL,                   -- 对公流水月均（元）
    ...
    -- V1轻复制新增字段（V0可不实现）
    source_partner_id VARCHAR(20),  -- 来源中介ID
...
| 字段 | 类型 | 说明 | V0 | V1 | 示例 |
|------|------|------|----|----|------|
| source_partner_id | VARCHAR(20) | 来源中介ID | ❌ | ✅ | M001 |
| source_channel | VARCHAR(50) | 来源渠道 | ❌ | ✅ | wechat_moments/h5_link |
| source_url | VARCHAR(500) | 来源URL | ❌ | ✅ | domain.com?partner_id=M001 |

**用途**：
- 追踪客户来源（朋友圈海报/H5链接/小程序）
- 数据归属清晰（客户属于哪个中介）
- 分析转化率（哪个渠道转化率高）
...
### 2.4 result_feedback（结果回填表）⭐⭐⭐⭐⭐

**功能定位**：数据飞轮核心，决定系统能否"越用越准"
...
CREATE TABLE result_feedback (
    ...
    reject_reason VARCHAR(50),  -- 拒绝原因（六宫格归因）
    predicted_amount DECIMAL(15, 2),  -- 系统预测额度
    actual_amount DECIMAL(15, 2),  -- 实际批复额度
    diff DECIMAL(15, 2),  -- 差异（actual - predicted）
...
| 字段 | 类型 | 说明 | 必填 | 示例 |
|------|------|------|------|------|
| result_id | VARCHAR(50) | 主键，结果ID | ✅ | R001 |
| customer_id | VARCHAR(50) | 客户ID | ✅ | C001 |
| product_id | VARCHAR(50) | 产品ID | ✅ | P001 |
| apply_date | DATE | 申请日期 | ✅ | 2026-01-20 |
| result | VARCHAR(20) | 申请结果 | ✅ | approved/rejected |
| reject_reason | VARCHAR(50) | 拒绝原因（六宫格） | ⚠️ 条件必填 | flow_insufficient |
| predicted_amount | DECIMAL(15,2) | 系统预测额度 | ❌ | 1000000.00 |
| actual_amount | DECIMAL(15,2) | 实际批复额度 | ❌ | 800000.00 |
| diff | DECIMAL(15,2) | 差异 | ❌ | -200000.00 |
| middleman_id | VARCHAR(50) | 中介ID | ✅ | M001 |
| notes | TEXT | 备注 | ❌ | 客户经理说... |
...
```python
REJECT_REASONS = {
    "flow_insufficient": "流水不足/不稳定",
    "invoice_insufficient": "开票不足",
    "tax_insufficient": "纳税不足",
    "credit_issues": "征信问题（查询多/负债高/有逾期）",
    "asset_insufficient": "资产不足（无抵押物）",
    "qualification_missing": "资质缺失（无高新等）",
    "industry_restricted": "行业限制",
    "other": "其他原因（需填写notes）"
}
```

**业务逻辑**：
1. 录入时机：中介申请产品后，结果出来时录入
2. 强制归因：如果result=rejected，必须选择reject_reason
3. 定时提醒：15天后自动提醒中介"客户XXX有结果了吗？"
4. 数据飞轮：
```
   推荐产品P001 → 10次申请 → 8次通过，2次拒绝
   分析拒绝原因 → 发现"流水波动>30%"的客户通过率低
   系统调整规则 → 流水波动>30% → 降低推荐优先级
   准确率提升：85% → 92%
```
...
### v1.4（2026-02-06）
**重大更新**：新增结果回填表 + V1轻复制字段

**核心变更**：
1. ✅ 新增表：result_feedback（结果回填表）⭐⭐⭐⭐⭐
   - 数据飞轮核心
   - 回填率<50%是生死线
   - 六宫格强制归因
   
2. ✅ tenants表新增字段（V1轻复制）：
   - partner_id（专属ID）
   - brand_name（品牌名称）
   - logo_url（Logo地址）
...
3. ✅ customers表新增字段（V1来源追踪）：
   - source_partner_id（来源中介ID）
   - source_channel（来源渠道）
   - source_url（来源URL）

**V0/V1划分**：
- V0：不实现轻复制字段（统一品牌"贷查查"）
- V1：实现轻复制字段（partner_id皮肤）
- V0必须实现：result_feedback表

**影响范围**：
- 开发团队：V0建表时新增result_feedback
- 产品团队：明确回填率目标（V0>50%, V1>80%）
