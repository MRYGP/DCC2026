...
**计划排期**（理想情况）：
- 功能开发：14天（Day 1-14，P0-14项功能）
- 测试优化：1周（Day 15-21）
- **目标工期**：3周完成V0闭环

**资源现实**（实际情况）：
- 若1-2名实习生（非全职/需要带教），则自然工期4-8周
- 原因：学习曲线、代码Review、质量闸门、返工
- **合理预期**：2-3个月完成V0闭环（含培训+开发+测试）
...
|----------|---------|--------------|-------------|------|
| **项目进度追踪器** | `00_核心文档/项目进度追踪器-v2.4.md` | v2.4 / 2026-02-09 | 第二章"V0开发计划"<br>技术栈部分 | V0已锁定：FastAPI/React+AntD/SQLite |
| **项目进度追踪器** | 同上 | v2.4 / 2026-02-09 | 第二章"V0开发计划"<br>V1新增功能预告部分 | V0不做：partner_id/海报/小程序/公众号 |
| **Skill格式规范** | `05_开发文档/Skill格式规范-v1.0.md` | v1.0（内容含v1.1变更记录） / 2026-02-12 | 第2章"通用元数据"<br>第3章"产品Skill" | Skill定义标准、热更新机制 |
| **13字段SSOT** | `05_开发文档/13字段字典-SSOT.md` | v1.0 / 2026-02-02 | 字段清单表 | **⚠️ 单位口径冲突待解决** |
...
**新人自查清单**：
- [ ] 能打开《项目进度追踪器v2.4》第二章"V0开发计划"
- [ ] 能打开《Skill格式规范v1.0》第2-3章
- [ ] 能打开《13字段字典-SSOT》字段清单表
...
**核心原则**：
- ❌ 不强制"前端/后端"岗位划分（避免技能单一化）
- ✅ 必须有**模块Owner**（避免无人负责）
- ✅ Owner负责：设计、实现、测试、文档、Code Review
- ✅ 备份人负责：理解代码、能接手、能Review

**为什么需要Owner？**
- 最大风险不是"慢"，而是"返工"和"质量失控"
- 上下文切换成本高（前端→后端→前端）
- 无Owner = 无人负责 = 质量失控
...
- [ ] 单元测试（至少3个用例）
- [ ] 文档更新（README/API文档）
- [ ] Code Review（审查备份人的PR）

**交叉覆盖规则**：
- 每周互换1个小任务（如A做前端一个小功能，B做后端一个小功能）
- 目的：理解对方代码，能接手
- 但不互换Owner（避免无人负责）
